...
suspend fun fxPerformTest(test: suspend CoroutineScope.()->Unit): Evolving<suspend CoroutineScope.() -> Unit> {
...
fun fxRunTest( test: suspend CoroutineScope.()->Unit): Unit = runBlocking {
    var m: Application? = null
        ...
        try {
            validate = fxPerformTest(test).get()
        }finally {
